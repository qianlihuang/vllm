...
template <typename scalar_t>
__global__ void minimax_moe_align_sort_kernel(
    const scalar_t* __restrict__ topk_ids,
    int32_t* __restrict__ sorted_token_ids, int32_t* __restrict__ expert_ids,
...
void moe_align_block_size(torch::Tensor topk_ids, int64_t num_experts,
  ...
  VLLM_DISPATCH_INTEGRAL_AND_UNSIGNED_TYPES(
      topk_ids.scalar_type(), "moe_align_block_size_kernel", [&] {
        ...
        bool minimax_align_sort_fast_path =
            !has_expert_map && num_experts == 256 && topk_ids.dim() == 2 &&
            topk_ids.size(1) == 8 && topk_ids.numel() <= 1024;
        bool small_batch_expert_mode =
            (topk_ids.numel() < 1024) && (num_experts <= 64);

        if (minimax_align_sort_fast_path) {
          constexpr int32_t threads = 1024;
          const int32_t shared_mem_size =
              (3 * num_experts + 1) * sizeof(int32_t);
          auto minimax_align_sort_kernel =
              vllm::moe::minimax_moe_align_sort_kernel<scalar_t>;
          minimax_align_sort_kernel<<<1, threads, shared_mem_size, stream>>>(
